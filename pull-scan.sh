...
# Scans the image
tmas scan -vv "$(if [ "$VULNERABILITY_SCAN" = true ]; then echo "-V"; fi)" "$(if [ "$MALWARE_SCAN" = true ]; then echo "-M"; fi)" "$(if [ "$SECRETS_SCAN" = true ]; then echo "-S"; fi)" -r "$REGION" registry:"$IMAGE" "$(if [ "$SBOM" = true ]; then echo "--saveSBOM"; fi)" >"$SCAN_RESULT_ARTIFACT"
